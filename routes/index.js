...
router.get('/teamResults/id/:teamID', function(req, res, next) {
  ...
  var teamID = req.params.teamID;
  //var query = "select p.name, p.club, p.age, p.nationality, p.overall from mydb.PlayerPersonalData p where p.age = "+ teamID + " order by p.overall desc";
  var query = "select result from (select * from( select 3 as result, date, match_api_id from Matches where (home_team_api_id = "+teamID+" and home_team_goal>away_team_goal) or (away_team_api_id = "+teamID+" and home_team_goal<away_team_goal) union select 0 as result, date, match_api_id from Matches where (home_team_api_id = "+teamID+" and home_team_goal<away_team_goal) or (away_team_api_id = "+teamID+" and home_team_goal>away_team_goal) union select 1 as result, date, match_api_id from Matches where (home_team_api_id = "+teamID+" and home_team_goal=away_team_goal) or (away_team_api_id = "+teamID+" and home_team_goal=away_team_goal) order by date desc) result1 limit 10) result2 order by date";
  console.log(query);
  connection.query(query, function(err, rows, fields) {
    if (err) console.log(err);
    else {res.json(rows);}
